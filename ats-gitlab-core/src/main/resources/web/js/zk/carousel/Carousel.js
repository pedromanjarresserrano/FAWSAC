zk.carousel.Carousel = zk.$extends(zk.Widget, {
    ...
    bind_: function () {
        ...
        var sliders = $("#"+this.uuid+"-carousel");
        var added  = this.domCarouselContent_();
        var aux = sliders.children();
    ...
    domCarouselContent_: function () {
        ...
        for (var b = obj.length, c = "", d = 0; d < b; d++) {
            c += '<span><img class="' + ((this.getLazyload() == "false") ? 'tns-image' : 'tns-lazy-img') + ' img-fluid " style="height: ' + obj[d].enlargedHeight + ';"' + ((this.getLazyload() == "false") ? 'src={' + obj[d].enlargedSrc + '}  ' : 'src="' + loadingImage + '"') + ((this.getLazyload() == "false") ? '' : 'data-src=' + obj[d].enlargedSrc) + '></span>';
        }
        return c;
